...
<div class="fixed inset-0 z-50 flex items-center justify-center">
	<!-- Backdrop -->
	<div class="absolute inset-0 bg-overlay" role="presentation" onclick={onclose}></div>

	<!-- Dialog -->
	<div
		class="relative z-10 w-full max-w-md rounded-lg border border-border bg-surface p-6 shadow-xl"
	>
		...
		{#if error}
			<div
				role="alert"
				class="mt-4 rounded-lg border border-danger/20 bg-danger-bg px-4 py-3 text-sm text-danger"
			>
				{error}
			</div>
		{/if}
