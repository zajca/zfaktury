<script lang="ts">
	...
	async function handleMarkFiled() {
		if (!confirm('Opravdu chcete označit přehled ZP jako podaný?')) return;
		actionLoading = 'filed';
		error = null;
		try {
		...
		<div class="mt-4">
			...
			<div class="mt-3 flex flex-wrap gap-2">
				...
				<Button
					variant="secondary"
					disabled={true}
					title="XML generování zatím není k dispozici"
				>
					Generovat XML
				</Button>
				<Button
			...
			<p class="mt-2 text-xs text-muted">XML generování zatím není k dispozici pro zdravotní pojišťovnu.</p>
		</div>
