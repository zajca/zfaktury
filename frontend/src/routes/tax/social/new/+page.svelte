<script lang="ts">
	...
	import { socialInsuranceApi } from '$lib/api/client';
	...
	async function handleSubmit() {
		...
		try {
			const result = await socialInsuranceApi.create({
				year: form.year,
				filing_type: form.filing_type
			});
			goto(`/tax/social/${result.id}`);
		} catch (e) {
			toastError(e instanceof Error ? e.message : 'Nepodařilo se vytvořit přehled pro ČSSZ');
		} finally {
			saving = false;
		}
	}
...
<svelte:head>
	<title>Nový přehled OSVČ pro ČSSZ - ZFaktury</title>
</svelte:head>

<div class="mx-auto max-w-2xl">
	<PageHeader title="Nový přehled OSVČ pro ČSSZ" backHref="/tax" backLabel="Zpět na daně" />
