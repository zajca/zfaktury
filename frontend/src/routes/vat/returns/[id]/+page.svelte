<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Nepodarilo se nacist priznani';
		} finally {
			loading = false;
		}
	}
	...
	async function handleDelete() {
		if (!confirm('Opravdu chcete smazat toto priznani?')) return;
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Nepodarilo se smazat priznani';
		} finally {
			actionLoading = null;
		}
	}
...
<svelte:head>
	<title>{vatReturn ? `DPH ${formatPeriod(vatReturn)}` : 'DPH Priznani'} - ZFaktury</title>
</svelte:head>
...
<div class="mx-auto max-w-4xl">
	...
	{:else if vatReturn}
		...
		<div class="mt-4 flex items-start justify-between">
			<div>
				<h1 class="text-2xl font-bold text-gray-900">DPH Priznani - {formatPeriod(vatReturn)}</h1>
			...
			<div class="flex flex-wrap gap-2">
				...
				>
					{actionLoading === 'recalculate' ? 'Prepocitavam...' : 'Prepocitat'}
				</button>
				<button
				...
				{#if vatReturn.has_xml}
					<button
						...
						class="rounded-lg border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
					>
						{actionLoading === 'download' ? 'Stahuji...' : 'Stahnout XML'}
					</button>
				{/if}
				{#if vatReturn.status !== 'filed'}
					...
					>
						{actionLoading === 'filed' ? 'Oznacuji...' : 'Oznacit za podane'}
					</button>
					<button
		...
		<div class="mt-6 space-y-6">
			<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
				<h2 class="text-lg font-semibold text-gray-900">Vystupni DPH</h2>
				<div class="mt-4 overflow-x-auto">
					<table class="w-full text-left text-sm">
						<thead class="border-b border-gray-200">
							<tr>
								<th class="pb-2 font-medium text-gray-600">Sazba</th>
								<th class="pb-2 text-right font-medium text-gray-600">Zaklad dane</th>
								<th class="pb-2 text-right font-medium text-gray-600">Dan</th>
							</tr>
						</thead>
						...
						<tfoot class="border-t border-gray-300">
							<tr>
								<td class="py-2 font-semibold text-gray-900">Celkem vystupni DPH</td>
								<td></td>
			...
			{#if vatReturn.reverse_charge_base_21 > 0 || vatReturn.reverse_charge_base_12 > 0}
				<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
					<h2 class="text-lg font-semibold text-gray-900">Preneseni danove povinnosti</h2>
					<div class="mt-4 overflow-x-auto">
						<table class="w-full text-left text-sm">
							<thead class="border-b border-gray-200">
								<tr>
			...
			<!-- Input VAT -->
			<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
				<h2 class="text-lg font-semibold text-gray-900">Vstupni DPH</h2>
				<div class="mt-4 overflow-x-auto">
					<table class="w-full text-left text-sm">
						<thead class="border-b border-gray-200">
							<tr>
						...
						<tfoot class="border-t border-gray-300">
							<tr>
								<td class="py-2 font-semibold text-gray-900">Celkem vstupni DPH</td>
								<td></td>
			...
			<!-- Result -->
			<div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
				<h2 class="text-lg font-semibold text-gray-900">Vysledek</h2>
				<div class="mt-4 flex flex-col items-end gap-2 text-sm">
					<div class="flex gap-8">
						<span class="text-gray-600">Vystupni DPH:</span>
						<span class="font-medium text-gray-900">{formatCZK(vatReturn.total_output_vat)}</span>
					</div>
					<div class="flex gap-8">
						<span class="text-gray-600">Vstupni DPH:</span>
					...
					<div class="flex gap-8 border-t border-gray-200 pt-2 text-base">
						<span class="font-semibold text-gray-900">{vatReturn.net_vat >= 0 ? 'Vlastni danova povinnost:' : 'Nadmerny odpocet:'}</span>
			...
			<div class="text-xs text-gray-400">
				Vytvoreno: {new Date(vatReturn.created_at).toLocaleDateString('cs-CZ')} |
				Upraveno: {new Date(vatReturn.updated_at).toLocaleDateString('cs-CZ')}
				{#if vatReturn.filed_at} | Podano: {new Date(vatReturn.filed_at).toLocaleDateString('cs-CZ')}{/if}
			</div>
		</div>
	{/if}
</div>
