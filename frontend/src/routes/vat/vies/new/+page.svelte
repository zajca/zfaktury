<script lang="ts">
	...
	const quarters = [
		{ value: 1, label: 'Q1 (leden - brezen)' },
		{ value: 2, label: 'Q2 (duben - cerven)' },
		{ value: 3, label: 'Q3 (cervenec - zari)' },
		{ value: 4, label: 'Q4 (rijen - prosinec)' }
	];
	...
	async function handleSubmit() {
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Nepodarilo se vytvorit souhrnne hlaseni';
		} finally {
			saving = false;
		}
	}
...
<svelte:head>
	<title>Nove souhrnne hlaseni - ZFaktury</title>
</svelte:head>

<div class="mx-auto max-w-xl">
	<a href="/vat" class="text-sm text-blue-600 hover:text-blue-800">&larr; Zpet na DPH</a>
	<h1 class="mt-2 text-2xl font-bold text-gray-900">Nove souhrnne hlaseni</h1>
