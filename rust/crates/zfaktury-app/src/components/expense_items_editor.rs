use gpui::*;
use zfaktury_domain::{Amount, ExpenseItem};
...
/// Event emitted when any expense item field changes.
pub struct ExpenseItemsChanged;

/// A single row in the expense items editor.
struct ItemRow {
...
/// Reusable expense items editor component.
/// Allows adding, removing, and editing expense line items with live totals.
pub struct ExpenseItemsEditor {
    items: Vec<ItemRow>,
}

impl EventEmitter<ExpenseItemsChanged> for ExpenseItemsEditor {}
...
impl ExpenseItemsEditor {
    ...
    /// Convert current editor state to domain ExpenseItem structs.
    /// Sets id=0, expense_id=0, vat_amount=ZERO, total_amount=ZERO
    /// because the service layer calculates those.
    pub fn to_expense_items(&self, cx: &App) -> Vec<ExpenseItem> {
        self.items
            .iter()
            .enumerate()
            .map(|(i, row)| {
    ...
    /// Populate items editor from existing expense items (for edit mode or OCR).
    pub fn set_items(&mut self, items: &[ExpenseItem], cx: &mut Context<Self>) {
        self.items.clear();
        for item in items {
    ...
    fn create_item_row(&mut self, cx: &mut Context<Self>) -> ItemRow {
        ...
        let quantity = cx.new(|cx| {
            NumberInput::new(SharedString::from(format!("exp-item-qty-{idx}")), "1", cx)
                .with_value("1")
        });
        cx.subscribe(
            &quantity,
        ...
        let unit = cx
            .new(|cx| TextInput::new(SharedString::from(format!("exp-item-unit-{idx}")), "ks", cx));
        cx.subscribe(
            &unit,
            |this: &mut Self, _entity, _event: &TextChanged, cx| {
                cx.emit(ExpenseItemsChanged);
        ...
        let unit_price = cx.new(|cx| {
            NumberInput::new(
                SharedString::from(format!("exp-item-price-{idx}")),
                "0.00",
                cx,
            )
        });
        ...
        let vat_rate = cx.new(|cx| {
            let mut select = Select::new(
                SharedString::from(format!("exp-item-vat-{idx}")),
                "DPH",
                vat_options(),
            );
...
impl Render for ExpenseItemsEditor {
            ...
            if can_remove {
                row_div = row_div.child(
                    div()
                        ...
                        .child(
                            div()
                                .id(SharedString::from(format!("rm-exp-item-{index}")))
                                .text_sm()
