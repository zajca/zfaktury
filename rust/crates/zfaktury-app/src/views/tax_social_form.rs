...
use zfaktury_domain::{Amount, FilingStatus, FilingType, SocialInsuranceOverview};
...
/// Social insurance overview creation form.
pub struct TaxSocialFormView {
...
impl TaxSocialFormView {
    pub fn new(service: Arc<SocialInsuranceService>, cx: &mut Context<Self>) -> Self {
        use chrono::Datelike;
    ...
    fn save(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(async move |this, cx| {
            let result = cx
                .background_executor()
                .spawn(async move {
                    service.create(&mut sio)?;
                    Ok::<i64, zfaktury_domain::DomainError>(sio.id)
                })
                .await;
            ...
            this.update(cx, |this, cx| {
                this.saving = false;
                match result {
                    Ok(id) => cx.emit(NavigateEvent(Route::TaxSocialDetail(id))),
...
impl EventEmitter<NavigateEvent> for TaxSocialFormView {}

impl Render for TaxSocialFormView {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let mut outer = div()
            .id("tax-social-form-scroll")
            .size_full()
            .bg(rgb(ZfColors::BG))
            .p_6()
            .flex()
